...
import { Expense } from "../modules/models/Expense/expense";
...
app.post("/expenses", (req: Request, res: Response) => {
  ...
  if (!description || !cost || !paidById || !groupId || !date) {
    return res
      .status(400)
      .send({
        message: "Description, cost, paidById, groupId, and date are required",
      });
  }
...
app.get("/expenses/paid-by/:userId", (req: Request, res: Response) => {
  ...
  const userExpenses = expenses.filter(
    (expense) => expense.paidBy.id === Number(userId)
  );
...
app.get("/expenses/for-user/:userId", (req: Request, res: Response) => {
  ...
  const userExpenses = expenses.filter((expense) =>
    expense.group.users.some((user) => user.id === Number(userId))
  );
  res.json(userExpenses);
});
export default app;
